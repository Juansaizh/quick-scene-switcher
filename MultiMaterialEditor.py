...
def get_helpers_script_path():
    """Resolves the absolute path to MultiMaterialEditor_helpers.ms."""
    current_dir = ""
    ...
    helpers_path = os.path.join(current_dir, "MultiMaterialEditor_helpers.ms")
    if os.path.isfile(helpers_path):
        return helpers_path

    # Fallback search across sys.path
    for path in sys.path:
        candidate = os.path.join(path, "MultiMaterialEditor_helpers.ms")
...
def init_maxscript_helpers():
    """Loads and registers MAXScript helper functions from MultiMaterialEditor_helpers.ms."""
    if not rt:
        return
    helpers_path = get_helpers_script_path()
    if os.path.isfile(helpers_path):
        try:
            norm_path = helpers_path.replace("\\", "/")
            rt.fileIn(norm_path)
        except Exception as e:
            print("Error loading MAXScript helpers from {}: {}".format(helpers_path, e))
    else:
        print("Warning: MultiMaterialEditor_helpers.ms not found at: {}".format(helpers_path))
